...
12. **安全**：把页面 DOM、导入文件（HAR/cURL/JSON）、runtime message、storage 数据都视为不可信输入，在边界处校验；状态修改类消息必须经 `isTrustedSender` 校验；遵循最小权限，不擅自扩大 `permissions`/`host_permissions`。注意：头名/CRLF 判据住在 `utils/headerValidation.ts`（表单保存侧与导入侧共用），状态码钳制与请求体上限由 `proxyHandler.ts` 导出，三者分别由 `tests/requestGuard.test.ts`、`tests/proxyResponseGuard.test.ts` 覆盖；导出脱敏（`utils/exportSanitize.ts`）与搜索高亮（`utils/highlight.ts`，已无 `v-html`）各有单测；`messageRouter.ts` 的 `isTrustedSender`（为测试而导出）与它守的 gate 由 `tests/messageRouter.test.ts` 直接 import 真实现覆盖：状态修改类消息在不可信 sender 下必须同步拒绝、不触达存储、且 gate 先于参数校验；只读消息与 `PROXY_REQUEST` **不能**加 gate——内容脚本的 `sender.url` 就是页面 URL，加上去代理当场失效——页面来源的约束只能落在 `entrypoints/content.ts` 的桥接层，不要试图靠扩大 `STATE_MUTATING_TYPES` 来「修」它。
