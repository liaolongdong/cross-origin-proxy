...
- **URL 匹配测试工具** — Options 顶栏全局入口，输入任意 URL 实时预览命中的规则、重写后 URL、转发通道（DNR / SW）与被遮蔽的规则
...
- **Popup 快速面板** — 代理总开关、今日统计、最近请求，一键直达 Options 页面；「为当前页创建规则」自动预填当前页面的规则表单（复用已打开的 Options 标签页）；配置了自动关闭时展示剩余倒计时
- **国际化** — 中英文界面切换
