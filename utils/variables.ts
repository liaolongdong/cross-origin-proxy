 ...
 * 这道校验、手改 storage 也能造出这种规则，**变量事后被人从表里删掉**也算在这一格里——那三个来路
 * 造出的坏配置早就落盘了，闸门拦在这里只是把「少配了一把密钥」的提醒变成「这条规则永久只读」。
 * 比较按**名字**，不看它出现在哪个位点——但两份基线各自取哪个位点是承重的：未定义那道问「这个名字
 * 表里有没有值」，头与查询可以合并；WS 那道问「这个位点读不读得到变量表」，只能按查询参数一侧算。
 */
