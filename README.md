...
一个只涉及本机的提醒：请求日志会存下被代理的请求头与请求体，里面可能包含 token；数据不出机器，但分享 HAR 导出或截图之前请先清空日志。配置导出则默认开启「分享模式」，自动剔除 `Authorization`、`Cookie` 等头与 token 类查询参数——需要全量备份时取消勾选，那份文件就和日志一样敏感。
...
规则 200 条、日志最近 500 条（单条正文最多 32K 字符，超出会截断并标注原始长度）、请求体上限 10MB、延迟 0–60000 毫秒；Mock 与响应改写的状态码钳制在 200–599，否则前端构造不出合法的 `Response`。
