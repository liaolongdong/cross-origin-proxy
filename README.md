...
- **HAR 1.2** 导出抓到的请求，与配置导出共用同一个**分享模式**勾选（默认开启）：脱敏时逐条剔除请求/响应头里的 `Authorization`、`Cookie` 一类凭据与 URL 上的 token 类查询参数，正文照旧保留；取消勾选即全量导出；导入 HAR 会依据录制请求自动生成代理规则（新规则默认停用，确认后自行启用）
...
一个只涉及本机的提醒：请求日志会存下被代理的请求头与请求体，里面可能包含 token；数据不出机器，但截图或粘贴日志详情之前请先清一遍。配置导出与 HAR 导出默认都开着「分享模式」，自动剔除 `Authorization`、`Cookie` 等头与 token 类查询参数——需要全量备份时取消勾选，那两份文件就和日志一样敏感。
