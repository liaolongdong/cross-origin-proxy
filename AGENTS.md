...
| 拦截器自报统计            | `entrypoints/background/interceptorStats.ts`（按 tabId 的**内存** Map，最多 10 个标签页、与 `swProxied` 交叉校验、不落 storage）、`utils/interceptorStats.ts`（界面侧五态判据，与 `dnrSample` 同类适配器）                                                                                    |
