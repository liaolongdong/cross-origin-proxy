...
- **拦截器的 fetch 通道按运行时测**（`tests/interceptorFetch.test.ts`：假 `window` ＋ 真的 `main()`，断言的是「这一笔到底发出去了没有、以什么落定」，而不是「那几行字还在」）。XHR 与 WebSocket 两条通道仍只有源码契约（`syncXhrFallback` / `xhrReadonlyProps` / `wsCapabilitySurface`）——它们的假实例（readyState/load 事件、握手与帧语义）本身就会成为测试的主要风险源，刻意没补；再往那两条通道加行为时，优先想办法补运行时用例。
