...
| 主题                      | `utils/theme.ts` + `assets/theme/tokens.css`（`--cop-*`，6 主题 + light/dark/system；动效档 `--cop-duration-*` / `--cop-ease-*`）+ `utils/transitions.ts`（`withViewTransition` / `applyFlip` / 读令牌）                                                                                      |
...
- **DNR 命中统计**：`getMatchedRules` 近 5 分钟窗口，配额约每 10 分钟 20 次；采样出口只有 `entrypoints/background/dnrSampler.ts`（全局 60s TTL、按标签页 15s TTL、滑窗 18 次预算、超限 60s 退避，退避期返回 `stale: true` 的缓存而不是报错）。读端返回的是 `DnrSample`，UI 侧的五态判据只有 `utils/dnrSample.ts`：`fresh`/`stale`/`notApplicable`（无生效的网络层规则）/`unavailable`（读不到）/`pending`，「0 次」与「不知道」绝不能合并成一句，也绝不能渲染成 0；能不能把计数当命中数画出来，只问 `isDnrCountReadable`（`fresh`/`stale` 才是「有读数」），`hits: null` 统一表达「没有读数」。两个统计窗口（DNR 的近 5 分钟 vs SW 的自配置变更以来）**不可相加**，聚合入口 `utils/ruleStats.ts` 的 `groupHitStatsByRule` 返回 `{ net, ext }`，规则列表因此分两格显示、绝不相加；**每格只画属于自己那条通道的数**——通道逐条由 `isSimpleRule` 判定（刻意不经总开关，否则刚禁用的网络层规则会在滚动窗口里丢掉真实命中数），不适用的那一格画「—」并配一句点名通道的提示（`hitStatsNetNotApplicable` / `hitStatsExtNotApplicable`），网络层那一格在读不到时同样是「—」，两格都是「—」时整列折叠成「-」。**SW 那一本账现在有运行时覆盖**（`tests/swHitStats.test.ts`）：命中判据排在 `blocked` / mock / 重写这些分支**之前**（所以被阻断的那一笔算命中，而没匹配到任何规则与总开关关闭的两笔都不算），读数按 `ruleId` 归并、按命中数降序，`GET_SW_STATS` 那条同步回包经真 `messageRouter` 出到 options。只有 `entrypoints/background.ts` 里那句 `resetSwHitStats()` 只能按源码契约钉——`defineBackground` 收到函数时只返回 `{ main }`，导入 SW 入口永远不会注册监听器，所以「配置变更即作废两份读数」在全仓没有任何运行时对应物；它必须排在 `if (newConfig?.rules)` 守卫**之外**（`proxy_config` 被整体清掉时 `newValue` 是 `undefined`，写在守卫里就等于关掉代理还能看见历史命中）。
- **代理自动关闭**：总开关开启且配置时长时用 `chrome.alarms` 倒计时（跨 SW 重启持久化），到期自动关闭总开关。
- **主题**：`--cop-*` 令牌 + `data-theme`/`data-mode`；6 主题（sky/green/pink/mauve/orange/slate）+ light/dark/system；同步覆盖 Element Plus `--el-color-primary` 梯度。新增引用必须在 `assets/theme/tokens.css` 里已定义（未定义的 `var()` 让整条声明在计算值阶段静默失效），`-rgb` 通道令牌存的是空格分隔值、只能写成 `rgb(var(--cop-primary-rgb) / 15%)`；两条契约由 `tests/designTokens.test.ts` 守卫。首帧不闪：`initThemeSync()` 在挂载前用 `localStorage` 镜像键 `cop_theme`/`cop_mode` 同步打上标记（`storage.local` 仍是事实来源，镜像只是消掉首帧的默认主题），该契约由 `tests/themeMirror.test.ts` 守着。**主题名与图标名同时住在两侧**：`THEME_NAMES` 里除默认主题（住在 `:root` 基座，刻意没有 `[data-theme='sky']`）外的每个名字都要在 `assets/theme/tokens.css` 有同名覆盖块，`THEME_MODE_OPTIONS[].icon` 要在 `components/options/SettingsDialog.vue` 的 `components` 注册表里——两边不一致是静默的（主题名对不上时色板**照样点得中**，只是根元素挂上一个没有样式匹配的属性值、画出来仍是 `:root` 那套默认配色；图标名对不上注册表就解析出一个空白，控制台都不响），对账与 `getStoredTheme`/`getStoredThemeMode` 那份「对调用方永不抛、非法值回落默认」的契约同由 `tests/themeSurface.test.ts` 守着；那两个 getter 的兜底与镜像那条「读不到 ≠ 存的就是默认值」的区分**刻意相反**——区分是给镜像用的（读不到就不许回写），调用方只想要一个画得出来的值。
- **动效层**：时长与缓动**只在** `assets/theme/tokens.css` 的 `--cop-duration-*`（instant 90 / fast 150 / base 220 / slow 320，另有一档 `--cop-duration-recolour` 280 专供换肤）与 `--cop-ease-*`（standard / enter 进场 / exit 离场 / spring 过冲）里；组件里再写字面量就是让同一种动作在两个界面差 130ms 的那条路。例外两处，别顺手「统一」：`tokens.css` 自己那 8 条（`::view-transition-*` 的 `var()` 兜底 + 降级段落的 `0.01ms` / `0s`）不是阶梯值，而 `components/` + `entrypoints/` 里刻意留着的 5 条是节拍与闪光时长（三档 `animation-delay` 60/120/180ms、`log-row-arrive` 0.7s、`url-diff-in` 0.5s）；`docs/assets/landing.css` 是独立产品站、不加载令牌层那 chunk，维持自己那套 `lp-` 前缀的字面量、一条 `--cop-*` 都不读。分档口径是「这次变化离用户的手多近」（按压要跟手、悬浮不突兀、面板与行列进场要被看清），缓动成对使用（进 enter / 出 exit / 一个 transition 承担两个方向才 standard），spring 只给「物体自己归位」的动作。JS 侧要读同一份值走 `utils/transitions.ts`（`readMotionToken` / `motionTokenToMs` / `prefersReducedMotion` 实时媒体查询 / `withViewTransition` / `readRects` + `applyFlip`），不要第二份时长表——该模块自己按**外部可观察面**有运行时覆盖（`tests/motionTokens.test.ts`：传给 `el.animate` 的实参、`update()` 在一条正常过渡里恰好被跑一次（交给 `startViewTransition` 的是包过一次的回调，不是执行结果）、`startViewTransition` 同步抛与「过渡被引擎**当场判死**、回调一次都没跑」这两条判死路径各补一次直接执行、回调跑过之后才被拒不许补第二次、reduce 那一档下 `animate` 一次都不发、令牌读不到才落字面量兜底）。`document.startViewTransition` 的真实快照合成与 CSS 侧观感在全仓测试里没有对应物；2026-09-26 用一支一次性真机探针（`.test-tmp/verify-motion-live.cjs`，Chrome for Testing 152 + CDP，文档最早期注入 8ms 轮询器读 `getAnimations()` 的 resolved timing，不入库）量过 21 项：弹窗/抽屉进 320 出 150、FLIP 320/spring、popup 自有过渡全在档、reduce 把过渡压到 0.01ms 且换肤不起 VT（配色照样改）。三种文档态也是这一轮量出来的：not fully active（把 iframe 从文档摘下）返回句柄、回调一次不跑、`ready` 与 `updateCallbackDone` 双双 `AbortError`——正是判死接手要接的那种；**hidden 页 VT 照常完成、回调照跑，但 `ready` 以 `InvalidStateError` 落回**（所以那两条 `.catch` 谁都不能省，省掉就是未处理拒绝）；frozen 页事件循环停摆（2024ms 窗口心跳只走 1 拍），那一档根本没有页面侧 JS 可测——注意调试器求值仍能穿过停摆的循环，那是 CDP 旁路，不是页面恢复了执行。**六处静默陷阱**：① 进场类 `animation` 一律 `backwards` 或不写 fill——`both` / `forwards` 会把 `transform` 冻在末帧，而动画在级联里排在 `:hover` 之前，悬浮抬升当场失效且无报错；② 引用了不存在的关键帧同样无报错（元素直接停在静止态），所以 `tests/designTokens.test.ts` 现在按「同一文件或 tokens.css 里有 `@keyframes`」全仓核对，含 `docs/assets/landing.css`；③ `transition: all` 全仓禁（同文件守卫，注释里点名历史写法的除外），它把宽高与内边距一并纳入过渡，抽屉/列表一改筛选就滑出一段并不存在的动画，规则表的行还要被 FLIP 的 `transform` 驱动、`all` 会抢同一件事；守卫按**整条声明**扫而不是按行——prettier 会把过渡清单折成 `transition:` + 换行列值，逐行匹配时 `all 0.2s ease` 正好从缝里过去；④ 覆盖 Element Plus 节奏的那几条**不得靠「我的样式表排在后面」赢**：它们编译进 options 首屏最后一个 `<link>`，而 EP 组件自己的 CSS 跟着弹窗/抽屉那几个**异步 chunk** 在运行时追加到 `<head>` 末尾，同特异度时后到者赢，于是「写了覆盖」与「覆盖生效」是两件事、且失效静默（2026-09-26 按真实产物在真 Chrome 里量过：8 条里只有 3 条生效，其余全是 EP 自带的 `all 0.3s`）。修法是把每条选择器抬到严格高于单类名（`entrypoints/options/styles.css` 的 `body ` 前缀），胜负从此与注入顺序无关；`tests/designTokens.test.ts` 按「凡覆盖 `.el-*` 的 transition/animation，选择器不得只是单个类名」守着，并把查了几条钉住（整体失配会静默假通过）；⑤ 错峰阶梯每一级写成 `:nth-child(n + k)`（「第 k 张起」）而不是逐个点名——逐个点名在今天完全正确，将来从 `templates` 里多加一张卡，它没有规则命中、延迟回落到 0、跟第一张同时进场，阶梯走到末尾倒着跳一次且无报错；守卫量到「再多一张」并把两叠卡的张数下限钉住，解析失效时先红；⑥ 换肤那一次 VT 有**四层**快照（`::view-transition-group` / `-image-pair` / `-old` / `-new`），少点一层它就退回 UA 自己的 `250ms / ease`，而点到的那几层是 280ms/standard——同一次点击两种节奏，且这件事**看不见**：整页快照不发生形变，仓库里也没有第二个 `view-transition-name`，只有 resolved timing 说得出（2026-09-26 正是这么抓到 `group` 那一层的；作者声明压得住它，同轮用 333ms 哨兵实测过），守卫是 `tests/designTokens.test.ts` 的「换肤快照的四层」——它按**每一条点名快照层的顶层规则**逐条过（自己按大括号深度切规则，`@media` / `@supports` / `@keyframes` 整块算一条），要求凡出现 `::view-transition-*` 的选择器必须是那四个 `(root)` 层、且所在规则同时声明 recolour 时长与 standard 缓动，最后核对四层都被点名过。收紧的原因是三种写法能骗过「文件里某处提到过这一层」这种判据：把某一层挪进 `@media`、在后面追加一条把同一层改回 250ms 的规则、给第二层起个非 root 的名字，三种各插一次变异都验证过会红（代价是把那两条属性等价改写成 `animation:` 简写也会红——简写的缓动默认值正是 `ease`）。同一轮还钉住了**读法本身**：CSSTransition 的缓动挂在 `getTiming()` 上，CSSAnimation 的挂在关键帧与 computed 上，`getTiming().easing` 对动画恒为 `linear`——按前者判弹窗进场会把「生效」读成「没生效」。`prefers-reduced-motion` 那段除时长外**必须连 `transition-delay` / `animation-delay` 一起归零**（否则错峰进场只是延迟闪一下），但 VT 那一层没有 CSS 开关可摘——`::view-transition-*` 的 animation 归零会留一段白屏空档，比不淡入更糟——由 `withViewTransition` 在 JS 侧直接跳过。换肤与换语言的整页淡入是 `applyThemeVisually` / `applyThemeModeVisually` / `setLocaleValue` 三个出口包的 VT，三者都经 `utils/theme.ts` 的 `requestVisual`（i18n 那份是自己的 `pendingLocale`）：**一次操作只淡一次**——判据不能只比根元素当前值，存储回声（`storage.onChanged` 绕回发起页自己）赶在过渡回调之前时旧值还挂在根元素上，于是回声被当成一次真变更、第二次过渡当场顶掉正在淡的那一次，代价是多拍一张整页快照加一条 `AbortError: Transition was skipped` 的未处理拒绝（2026-09-26 真实 Chrome 实测：一次点击一条），所以要同时看「已经有一次在途的过渡正把画面画成什么」；在途目标对不上时照常淡第二次（中途改口不能被当成回声吞掉），收账只收自己那一份（先跑的回调不许抹掉后一笔的账），`withViewTransition` 则接手 `ready` / `finished` 那两条 promise——它们被顶掉时**正常**就拒绝。这份时序由 `tests/motionEcho.test.ts` 按外部可观察面钉（假 `startViewTransition` **推迟**回调，跟真实浏览器一样；变异实测：摘掉去重、摘掉任一处接手、把监听器的 `mode` 丢了、摘掉 `updateCallbackDone` 那条判死接手、摘掉 `startViewTransition` 外层的 catch、摘掉 `ran` 旗标（让回调可能被跑两遍）都变红）。**首帧那几次仍同步直写**（镜像键存在的全部理由），且不得碰「镜像只在 `storage.local` 落盘成功之后刷新」那条不变量（见上面主题一条与 `tests/{themeMirror,localeMirror}.test.ts`）。
