 ...
 * 最慢的那一页可以把时间拖到秒级，而这期间别的页完全可能已经刷新。逐页结算让常见的时序是
 * 「先标记、后清账」——老文档销毁时标记落下，新文档的 `loading` 导航与它自己的拉取
 * （`GET_PROXY_CONFIG` 也清账）随后把这笔账抹掉。反过来的那一格没有兜底：一句回绝落在哪一刻
 * 由那一页自己的信道决定，代码这边给它不上界，所以它完全可能晚于新文档的拉取，把已经同步的
 * 一页又盖成假警告，只能等下一次导航、或下一次广播被它接住才消失。
 * 两次广播叠在一起是同一形状（这里刻意没像 `syncDnrRules` 那样排队，前一次迟到的回绝能盖掉
 * 后一次回执的清账）。要堵它得把清账分成「证明已同步」（回执、拉取）与「只是复位」（导航）
 * 两档，只让前者作废迟到的标记——后者什么也没证明，扩展被该站点禁用时新文档压根不会来拉。
 *
